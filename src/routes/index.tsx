...
function HomePage() {
  ...
  return (
    <>
      ...
      {/* SLIDE 2: SERVICES */}
      <section className="homepage-section bg-background py-[clamp(2.4rem,4.8vh,4.8rem)] min-h-screen flex items-center [scroll-snap-align:start]">
        <div className="homepage-wide w-full flex flex-col justify-center">
          <div className="max-w-none mb-[clamp(1.2rem,3vh,2.4rem)]">
            <p className="font-mono text-[13px] uppercase tracking-[0.3em] text-gold mb-2.5">
              {t("services.eyebrow")}
            </p>
            <h2 className="font-display text-[clamp(1.5rem,2.4vw,2.7rem)] leading-tight">
              {t("services.title.line1")}<br />
              {t("services.title.line2")}
            </h2>
            <div className="gold-divider w-24 mt-4" />
            <p className="text-base md:text-lg text-muted-foreground mt-4 max-w-[70ch]">
              {t("services.lede")}
            </p>
          </div>
            ...
            {services.map((s, i) => {
              ...
              return (
                <Link
                  key={s.titleKey}
                  to="/services"
                  className={`group relative p-[clamp(1.2rem,1.9vw,2.1rem)] min-h-[clamp(11rem,20vh,17rem)] transition-all hover:shadow-premium ${
                    isDark ? "bg-ink text-white hover:bg-ink-soft" : "bg-card text-foreground hover:bg-secondary"
                  }`}
                >
                  <div className={`size-11 grid place-items-center mb-4 border ${isDark ? "border-gold/30 text-gold" : "border-gold text-gold"}`}>
                    <Icon size={22} strokeWidth={1.5} />
                  </div>
                  <h3 className="font-display text-xl xl:text-2xl mb-2">{t(s.titleKey)}</h3>
                  <p className={`text-sm xl:text-base leading-snug line-clamp-3 ${isDark ? "text-white/65" : "text-muted-foreground"}`}>
                    {t(s.textKey)}
                  </p>
                  <ArrowUpRight
                    size={19}
                    className="absolute top-5 right-5 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1 text-gold"
                  />
                </Link>
              );
            })}
